...
namespace ProjectX.StateMachine.Editor
{
    public class TransitionTableEditorWindow : EditorWindow
    {
        ...
        [MenuItem("Transition Table Editor", menuItem = "Project X/Transition Table Editor")]
        ...
        private void CreateListView()
        {
            ...
            if (_transitionTableEditor && _transitionTableEditor.target)
                listView.selectedIndex = Array.IndexOf(assets, _transitionTableEditor.target);
        }

        private void OnListSelectionChanged(IEnumerable<object> eList)
        {
            List<object> list = eList.ToList();
